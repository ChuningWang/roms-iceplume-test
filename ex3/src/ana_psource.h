...
!
        ...
        IF (Master.and.DOMAIN(ng)%SouthWest_Test(tile)) THEN
          ...
          DO is=1,Nsrc(ng)
            ...
            SOURCES(ng)%Jsrc(is)=24+is
# if defined ICEPLUME
            SOURCES(ng)%SGdep(is)=-260.0_r8
            IF (is.eq.NINT(USER(3))) THEN
...
#  ifdef ICEPLUME_DET_AVERAGE
            SOURCES(ng)%SGIrange(is,1)=2.0
            SOURCES(ng)%SGIrange(is,2)=7.0
            SOURCES(ng)%SGJrange(is,1)=30.0
            SOURCES(ng)%SGJrange(is,2)=38.0
#  endif
# endif
          END DO
        END IF
...
#   if defined ICEPLUME_DET_AVERAGE
...
!$OMP BARRIER
        ...
        IF (DOMAIN(ng)%NorthEast_Test(tile)) THEN
          ...
          END DO
# if defined ICEPLUME
          fac=MAX(TANH(pi*(tdays(ng)-dstart-200.0_r8)*0.1_r8), 0.0_r8)
          DO is=1,Nsrc(ng)
            IF (is.eq.NINT(USER(3))) THEN
              SOURCES(ng)%SGbar(is)=fac*USER(4)
            ELSE
        ...
        IF (DOMAIN(ng)%NorthEast_Test(tile)) THEN
          DO k=1,N(ng)
            DO is=1,Nsrc(ng)
              ...
              DO itrc=3,NT(ng)
                SOURCES(ng)%Tsrc(is,k,itrc)=0.0_r8
              END DO
            END DO
          END DO
#  if defined ICEPLUME
