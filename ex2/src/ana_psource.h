...
!
      integer :: Npts, NSUB, is, itrc, i, j, k, ised
...
!
        ...
        IF (Master.and.DOMAIN(ng)%SouthWest_Test(tile)) THEN
          Nsrc(ng)=1
          my_mid=NINT(REAL(Mm(ng),r8)/2.0_r8)
          DO is=1,Nsrc(ng)
            SOURCES(ng)%Dsrc(is)=0.0_r8
            SOURCES(ng)%Isrc(is)=2
            SOURCES(ng)%Jsrc(is)=my_mid
...
# ifdef ICEPLUME_DET_AVERAGE
          DO is=1,Nsrc(ng)
            ...
            SOURCES(ng)%SGJrange(is, 1)=my_mid-2
            SOURCES(ng)%SGJrange(is, 2)=my_mid+3
          END DO
# endif
        END IF
#else
...
!$OMP BARRIER
        ...
        DO is=1,Nsrc(ng)
          ...
          IF (SOURCES(ng)%Jsrc(is).EQ. my_mid) THEN
